...
| `flat_beam_noise_floor.tsv` (Table 1, Fig. 1) + `nf_seed_*` | Octopus `validation/noise_floor_meshswap.jl` | A100 preferred | production `picgrid/hybgrid = 128/64` assignment reproduces Table 1 (verified bit-for-bit at submission); seeds per archived set |
| `noise_floor_meshswap.tsv` (Sec. 4.1 control) | same script, meshswap assignment | A100 preferred | |
...
| `mesh_study_reexecution.tsv`, `mesh_study_both_planes.tsv`, `mesh_study_cache_none.tsv` | `drivers/mesh_study_driver.jl` | A100 | cache_none arm: `OCTOPUS_PIC_GREEN_CACHE=none`; seeds 1/2222/3333 |
| `lambda_round_converged.tsv`, `lambda_flat_converged.tsv` (Table 2) | Octopus `validation/lambda_round_converged.jl` / `lambda_flat_converged.jl` | A100, long | 8192 turns, 1e5/beam, 3 seeds each |
...
| `yokoya_vs_aspect_measured.tsv`, `yokoya_vs_xi_measured.tsv`, `lambda_narrowplane.tsv` | `drivers/lambda_flatxi.jl`, `drivers/lambda_fixedxiy.jl`, `drivers/lambda_narrowplane.jl`, `drivers/lambda_tunescan.jl` | A100, long | scans print to stdout — capture transcripts to `data/*.log` and transcribe; keep the transcripts (see README, Run transcripts) |
| `eic_emittance_octopus.tsv` (Table 3, Fig. 8) | Octopus `validation/eic_emittance_benchmark.jl` | A100, hours | BB3D side stays frozen unless BeamBeam3D is rerun |
| `emit_xcode_*.tsv`, centroid overlays | Octopus `validation/emit_xcode.jl` | A100 | |
...
and the BeamBeam3D outputs (`multislice_centroids_bb3d*`, `eic_emittance_bb3d`,
`data/bb3d_decks/*fort*`). The zscan jumps regenerate only if
`slice_longitudinal_zscan.jl` (in Octopus validation) still emits the jump
tables; check before assuming. BB3D reruns need BeamBeam3D itself; the
manuscript rests no conclusion on the frozen-only files alone.

## Performance datasets (A100 production machine ONLY, regenerate last)
...
2. A100 physics campaigns (Tables 1–3, Figs. 5–9 measured sides).
